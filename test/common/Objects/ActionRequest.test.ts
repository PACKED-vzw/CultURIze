...
describe("ActionRequest", () => {
    it("ActionRequest construction", () => {
        const ar = new ActionRequest(Action.publish, "csv", "dir", "url", "branch", "commitmsg",
                                     "prtitle", "prbody", Target.apache);
        ...
        expect(ar.prTitle).to.eql("prtitle");
        expect(ar.prBody).to.eql("prbody");
        expect(ar.user).to.be.undefined;
        expect(ar.target).to.eql(Target.apache);
    });

    it("PublishRequest functions", () => {
        const ar = new ActionRequest(Action.validate, "csv", "dir", "url", "branch", "commitmsg",
                                     "prtitle", "prbody", Target.nginx);
